...
void add_resolution(uint32_t width, uint32_t height)
{
	...
	{
		struct resolution *res = &resolution_list[i];
...
void fl2000_monitor_ratio_to_dimension(uint8_t x, uint8_t aspect_ratio, uint32_t *width, uint32_t *height)
{
	uint32_t temp_width;
...
void test_display_all(int fd)
{
	...
	{
		struct resolution *res = &resolution_list[i];
		test_display(fd, res->width, res->height, 0);
	}
}
